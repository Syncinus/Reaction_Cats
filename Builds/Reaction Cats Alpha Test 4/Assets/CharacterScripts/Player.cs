...
public class Player : NetworkBehaviour {
    ...
    public void Start ()
	{
		...
		for (int i = 0; i < abilitysList.Count; i++) {
			//f (thingAbility != null) {
			Ability thingAbility = abilitysList.ElementAt(i);
		...
		if (Disabled == false) {
			...
			//	}
				...
				if (CATATTACKING == true && CurrentTurnsRecharging == 0) {
					...
					if (Vector2.Distance(currentHex.position, attackingHex.position) < 14) {
		...
		//}
			...
			*/
			...
			while (Vector3.Distance (transform.position, quad.transform.position) > 0.3) {
				...
				transform.position = Vector3.MoveTowards (properTransformPosition, quad.transform.position, TravelSpeed * Time.deltaTime);
    ...
    IEnumerator Attack()
    {
		...
		for (var n = 0; n < attackedhexes.Count; n++) {
			...
			quint.WobbleLikeTheWeebles ();
			plasma.ShootPlasma (this.transform.position, quint.transform.position);
		}
        yield return new WaitForSeconds(0.2f);
		CATATTACKING = false;
