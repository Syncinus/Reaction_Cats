...
public class NeoHexGenerator : MonoBehaviour {
	/*
	public static NeoHexGenerator inst;
	...
	//Internal variables
	private Dictionary<string, Hex> grid = new Dictionary<string, Hex>();
	...
	public Hex TileAt(CubeIndex index){
		if(grid.ContainsKey(index.ToString()))
			return grid[index.ToString()];
		return null;
	}

	public Hex TileAt(int x, int y, int z){
		return TileAt(new CubeIndex(x,y,z));
	}

	public Hex TileAt(int x, int z){
		return TileAt(new CubeIndex(x,z));
	}

	public List <Hex> Neighbours(Hex tile) {
	...
	public List<Hex> Neighbours(CubeIndex index){
		return Neighbours(TileAt(index));
	}

	public List<Hex> Neighbours(int x, int y, int z){
		return Neighbours(TileAt(x,y,z));
	}

	public List<Hex> Neighbours(int x, int z){
		return Neighbours(TileAt(x,z));
	}

	public List<Hex> TilesInRange(Hex center, int range){
	...
	public List<Hex> TilesInRange(CubeIndex index, int range){
		return TilesInRange(TileAt(index), range);
	}

	public List<Hex> TilesInRange(int x, int y, int z, int range){
		return TilesInRange(TileAt(x,y,z), range);
	}

	public List<Hex> TilesInRange(int x, int z, int range){
	...
	private void GetMesh() {
		hexMesh = null;
		Hex.GetHexMesh(hexRadius, hexOrientation, ref hexMesh);
	}
	...
	private Hex CreateHexGO(Vector3 postion, string name) {
		GameObject go = new GameObject(name, typeof(MeshFilter), typeof(MeshRenderer), typeof(Hex));
		...
		if(drawOutlines) {
			...
			for(int vert = 0; vert <= 6; vert++)
				lines.SetPosition(vert, Hex.Corner(tile.transform.position, hexRadius, vert, hexOrientation));
		}
