...
public class Player : NetworkBehaviour {
    ...
    public bool PassiveDisable = false;
    public bool ActiveDisable = false;
	public Cat curcat;
    ...
    public bool finishedActionSelection = false;



    #region NewActionSystem
    ...
    public void FinishTurn(ActionInfo info)
    {
        playerAssignSystem.infoList.Add(info);
        finishedActionSelection = true;
    }
    ...
    public IEnumerator SelectMovement(int maxHexes, int curPhase)
    {
        int hexesSelected = 0;
        while (hexesSelected < maxHexes)
        {
            Transform moveHex = this.transform;
            ...
            {
                string propHexName = currenthexrow + " " + currenthexnum;
                if (hex.name == propHexName)
                {
                    currentHex = hex;
                }
            }
            foreach (Transform hexToMoveTo in hexmap)
            {
                string moveHexName = hri + " " + hni;
                if (hexToMoveTo.name == moveHexName)
                {
                    moveHex = hexToMoveTo;
                    terrHeight = moveHex.GetComponent<Hex>();
                }
            }
            if (Vector2.Distance(currentHex.position, moveHex.position) < 6f)
            {
                ...
                if (allowedToMove == true)
                {
                    foreach (Transform hex in hexmap)
                    {
                        string hexName = hri + " " + hni;
                        if (hex.name == hexName)
                        {
                            Hex hexComponent = hex.GetComponent<Hex>();

                            if (hexComponent.OnHex != "DesComp")
                            {
                                bool passability = true;

                                if (hexComponent.OnHex == "Terrain")
                                {
                                    TerrainScript tScript = hexComponent.objectOnhex.GetComponent<TerrainScript>();
                                    passability = tScript.IsPassable;
                                }

                                if (passability == true)
                                {
                                    if (hexComponent.OnHex != "Player" || hexComponent.objectOnhex == this.transform)
                                    {
                                        if (!hexes.Contains(hexComponent))
                                        {
                                            hexes.Add(hexComponent);
                                            hexesSelected += 1;
                                            myHex = hex;
                                            FollowScript.GoToPosition = hex.position;
			...
			//	}
				...
				if (CATINTERACTING == true) {
					...
					if (Input.GetKeyDown (KeyCode.C) && running == false) {
                        ...
                        if (playerAssignSystem.catlist.Count > 1)
                        {
    ...
    IEnumerator SwitchBack() {
        ...
        //ActionInitilized.stopThePassive = false;
        //ActionInitilized.stopTheActive = false;
        //ActionInitilized.disableActive = false;
        //ActionInitilized.disablePassive = false;
        //TurnSystem.passiveLocked = false;
		...
		//playerAssignSystem.DoingPassive = true;
		//splayerAssignSystem.DoingActive = false;
