...
public class HexGridGenerator : NetworkBehaviour {
	...
	public float spacerX = 0.02f; //wild hair addition | add a gap between hexes
	public float spacerY = 0f;
	...
	void Start() {
		hexWidth =  Hex.GetComponent<Renderer>().bounds.size.x + spacerX;
		hexHeight = Hex.GetComponent<Renderer>().bounds.size.y + spacerY;
	...
	public void CreateHexGrid() {
		...
		for (float y = 0; y < gridHeightInHexes; y++)
		{
			...
			for (float x = 0; x < gridWidthInHexes; x++)
			{
				...
				int orderThing = (int) y;
				hex.GetComponent<SpriteRenderer> ().sortingOrder = orderThing;
				hex.transform.position = new Vector3 (hex.transform.position.x, hex.transform.position.y + (y / 1.9f), hex.transform.position.z);
