...
//	}
	...
	public void AddCat(Cat CatToRecive, NetworkInstanceId netInstanceId) {
		...
		CmdAddCat (netInstanceId, catToAdd.name);
	}

	[Command] public void CmdAddCat(NetworkInstanceId netInstanceId, string CatsName) {
		RpcAddCat (netInstanceId, CatsName);
	}
	...
	if(Input.GetMouseButtonDown(0) && DoingSomething == false)
	{
		...
		else
		{
			...
			if (mouseSelection.gameObject.GetComponent<Player> ()) {
				...
				if (theCat.Team == Team) {
                        //PlayerTracker.player = theCat.transform;
                        PlayerTracker.m_Target = theCat.transform;
